// 自动提取vue国际化配置文件 ==> 输出到ouput文件夹(json)，操作的文件应该是vue
...
mkdirSync('output', 0, error => !error && fileDisplay(filePath) /*调用文件遍历方法*/)

function mkdirSync (url, mode = 755, cb) { // 创建文件夹
  const arr = url.split('/')
  cb = cb || function () {}
  if (arr[0] === '.') arr.shift() //处理 ./aaa
  if (arr[0] == '..') arr.splice(0, 2, arr[0] + '/' + arr[1]) //处理 ../ddd/d
...
function fileDisplay (filePath) { //文件遍历方法
  return fs.readdir(filePath, function (err, files) { //根据文件路径读取文件，返回文件列表
    if (err) return console.error(err)
    else {
      return files.forEach(function (filename) { //遍历读取到的文件列表
        const filedir = path.join(filePath, filename) // 当前文件，携带了路径
        fs.stat(filedir /*获取到当前文件的绝对路径*/,
          function (eror, stats) { //根据文件路径获取文件信息，返回一个fs.Stats对象
            if (eror) {
              ...
              if (isFile && !!~filename.indexOf('.vue')) { // 读取文件内容
                const content = fs.readFileSync(filedir, 'utf-8'),
                  res = /<i18n>([\S\s]*)<\/i18n>/.exec(content),
                  data = res && res[1]
                fs.writeFile(
                  path.join(__dirname,'output', filename.replace('.vue', '.json')), data,
